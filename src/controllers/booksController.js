...
export async function renderIndex(req, res) {
  const requestedSort = typeof req.query.sort === "string" ? req.query.sort : "recent";

  try {
    const { books, sort } = await findAllByUserId(req.user.id, requestedSort);
...
export async function updateBook(req, res) {
  ...
  try {
    ...
    return res.redirect(`/?sort=${normalizeSort(req.query.sort)}`);
  } catch (error) {
